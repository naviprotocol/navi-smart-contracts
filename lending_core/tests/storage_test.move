...
module lending_core::storage_test {
    ...
    #[test]
    public fun test_mint_owner_cap_from_storage_admin() {
        let recipient = @0x123;
        ...
        // Mint new OwnerCap for RECIPIENT
        test_scenario::next_tx(&mut s, OWNER);
        {
            let admin_cap = test_scenario::take_from_sender<StorageAdminCap>(&s);

            storage::mint_owner_cap(&admin_cap, recipient, test_scenario::ctx(&mut s));

            test_scenario::return_to_sender(&s, admin_cap);
        };

        // Verify RECIPIENT received the OwnerCap
        test_scenario::next_tx(&mut s, recipient);
        {
            assert!(test_scenario::has_most_recent_for_sender<OwnerCap>(&s), 0);
